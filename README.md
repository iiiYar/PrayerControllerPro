...
├── build.bat
└── PrayerMediaApp_Ultima.cs        # الملف القديم محفوظ مؤقتًا كمرجع
```
...
- محاولة ترحيل الإعدادات القديمة تلقائيًا عند أول تشغيل
...
- يمكن اختيار طريقة التحكم بالوسائط من الإعدادات: زر النظام `Play/Pause` القديم أو `Volume guard` الذي يخفض صوت التطبيقات الأخرى أثناء نافذة الصلاة ثم يعيدها بعدها.
- الملف القديم `PrayerMediaApp_Ultima.cs` لم يُحذف بعد حتى يبقى مرجعًا أثناء الانتقال.
- يمكن التبديل بين لوحة التحكم الكاملة ووضع `Widget Mode` من الزر داخل الواجهة.

## توثيق التغييرات

راجع [docs/whats-new.md](docs/whats-new.md) لملخص الميزات والإصلاحات التي أضيفت في هذه المرحلة.
